...
- `FastAPI + edge-tts` cho service Python stream audio qua WebSocket
...
- `tts_service/`: service Python realtime TTS, hiện runtime dùng `edge-tts` CLI
...
- Script này sẽ:
  ...
  - cập nhật `STORY_TTS_EDGE_BINARY` trong `backend/.env` về binary `edge-tts` bên trong venv của repo
...
- `tts_service` sẽ ưu tiên dùng `edge-tts.exe` trong `data/run/tts-venv/Scripts/` nếu file này tồn tại; nếu không mới fallback sang bản global trong `%AppData%`.
