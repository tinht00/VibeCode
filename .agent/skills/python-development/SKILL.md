...
*   **Vectorization**: Luôn ưu tiên vector operations thay vì loop Python thuần.
    *   ✅ *Tốt*: `df['price'] = df['price'] * 1.1`
    *   ❌ *Tệ*: `df.apply(lambda x: x['price'] * 1.1, axis=1)`
*   **Chaining**: Sử dụng method chaining (`.assign()`, `.pipe()`) để code dễ đọc.
*   **Memory Usage**: Sử dụng `category` dtype cho string columns lặp lại nhiều.

## Environment & Dependency

*   **Virtual Env**: `venv`, `conda` hoặc `poetry`.
*   **Lock files**: Sử dụng `poetry.lock` hoặc `requirements.txt` (với version cứng) để đảm bảo reproducibility.
