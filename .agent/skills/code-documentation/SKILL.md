...
### Ngôn ngữ comment
*   **Ưu tiên Tiếng Việt** cho phần giải thích logic phức tạp, hướng dẫn nghiệp vụ (Business Logic).
*   **Tiếng Anh** được chấp nhận cho các chuẩn chung (API Params, Return types).
*   Phải nhất quán trong một file. Không viết "nửa Anh nửa Việt" lộn xộn (trừ thuật ngữ chuyên ngành).
*   **Why > What**: Giải thích TẠI SAO, không phải đang làm gì.

### Golang Doc
```go
// CalculateTotal returns the final amount including tax.
//
// Nó sử dụng thuật toán làm tròn Banker's rounding để đảm bảo chính xác tài chính.
...
def calculate_total(subtotal: float, tax_rate: float) -> float:
    """Tính tổng giá trị đơn hàng sau thuế.

    Args:
        subtotal: Tổng tiền hàng chưa thuế.
        tax_rate: Thuế suất (0.1 = 10%).
...
### Inline Comment Tags
*   **TODO**: `// TODO(User): task description` — việc cần làm sau.
*   **FIXME**: `// FIXME: description` — lỗi cần sửa gấp.
*   **NOTE**: `// NOTE: ...` — lưu ý về behavior lạ.

### Commit Messages (Conventional Commits)
*   `feat: thêm chức năng login`
*   `fix: sửa lỗi crash khi nhập ngày sai`
*   `docs: cập nhật hướng dẫn API`
*   `refactor: tối ưu code xử lý chuỗi`

### API Documentation (Markdown)
*   **Yêu cầu bắt buộc**:
    *   **Full Request Body**: Liệt kê đầy đủ tất cả các trường, bao gồm cả optional.
    *   **Sample Response**: Cung cấp JSON response mẫu thực tế.
    *   **Table Description**: Giải thích ý nghĩa từng trường.
